...
public class ImpStaticManager implements Listener<Object>{
	 ...
	 */
	private HashMap<String,HostJobs> monitorAlljobstaticsIdxByHost = new HashMap<String,HostJobs>();
	/**
	 * Map<jobname,Map<hostname,JobStatic>>
	 */
	private HashMap<String,Map<String,JobStatic>> monitorAlljobstaticsIdxByJob = new HashMap<String,Map<String,JobStatic>>();
	


	private HashMap<String,Integer> monitorAlljobstatusIdxByJob = new HashMap<String,Integer>();
	...
	void handlecommand(Event<Object> e_)
	{
		...
		if(command.equals(HDFSUploadData.hdfs_upload_monitor_request_commond))
		{
		...
		else if(command.equals(HDFSUploadData.hdfs_upload_monitor_response_commond))
		{
			/**
...
//				specialMonitorObjects.clear();
				...
				if(oldalljobstatics == null || oldalljobstatics.getDatanodeTimestamp() < alljobstatics.getDatanodeTimestamp())
				{
		...
		else if(command.equals(HDFSUploadData.hdfs_upload_monitor_addworkthread_commond))
		{
			...
			if(nums != null)
			{
				new Thread(new Runnable(){
					public void run()
					{
						...
						log.info("["+addWorkthreads.getAdjustJobname()+"]"+nums+"." );
					}
				}).start();
			}
			else
			{
				...
				log.info("["+addWorkthreads.getAdjustJobname()+"]." );
			}
	...
	public String persistentMonitorObject(SpecialMonitorObject specialMonitorObject) throws Exception
	{
		if(SimpleStringUtil.isEmpty(specialMonitorObject.getJobstaticid() ))
			return "" +specialMonitorObject.getJobName() +"id,";
