...
//				}
			...
			{
				/**
				 * try resolved oracle number1.30020034599143E-115 row.getString(i+1)
...
//			}
	    	...
	    	if(fileSegment.job.config.datatype == null || fileSegment.job.config.datatype.equals("json"))
	    	{
				...
				for(int i =0; i < counts; i++)
				{
					...
					if("ROWNUM__".equals(colName))//oracle
						continue;
		...
		catch(NestedSQLException e)
		{
			...
			if(e.getCause() != null && e.getCause() instanceof RowHandlerException)
			{
				...
				{
					fileSegment.appendErrorMsg(SimpleStringUtil.formatException(e.getCause()));//
...
//					log.info(""+fileSegment.toString());
		 	...
		 	catch(NestedSQLException e)
		 	{
		 		...
		 		if(innere == null)
		 		{
		 			if(fileSegment != null)//
					{
						...
						fileSegment.taskStatus.setTaskInfo(fileSegment.toString());
						log.error(""+fileSegment.toString(),e);
					}
		 		...
		 		else if(innere instanceof ForceStopException)
		 		{
		 			if(fileSegment != null)//
			 		{
						...
						fileSegment.taskStatus.setErrorInfo("");
						fileSegment.taskStatus.setTaskInfo(fileSegment.toString());
			 		}
		 		}
		 		else
		 		{
		 			if(fileSegment != null)//
					{
						...
						fileSegment.taskStatus.setTaskInfo(fileSegment.toString());
						log.error(""+fileSegment.toString(),innere);
					}
			...
			finally
			{
				tm.release();
				this.buidler = null;
				if(fileSegment != null && !fileSegment.isClosed())//
				{
					fileSegment.handleerrormsgs();
					fileSegment.close();
				}
