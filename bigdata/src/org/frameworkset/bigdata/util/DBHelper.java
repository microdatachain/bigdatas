...
public class DBHelper {
	...
	public static void initConfgDB(String dbpath)
	{
		...
		try {
			...
			log.info("jobconfig table jobconfigcreate table jobconfig (jobname string, jobdef string,PRIMARY KEY (jobname))");
			try {
				SQLExecutor.updateWithDBName("bigdata_conf","create table jobconfig (jobname string, jobdef string,PRIMARY KEY (jobname))");
				log.info("jobconfigcreate table jobconfig (jobname string, jobdef string,PRIMARY KEY (jobname))");
			} catch (SQLException e1) {
				log.info("jobconfigcreate table jobconfig (jobname string, jobdef string,PRIMARY KEY (jobname))",e1);
				e1.printStackTrace();
			}
		}
...
//				SQLExecutor.updateWithDBName("bigdata_conf","drop table jobstatic ");
				...
				log.info("jobstatic table jobconfigcreate table jobstatic (jobstaticid string,jobname string, jobstatic TEXT,savetime number(10), PRIMARY KEY (jobstaticid))");
				try {
					...
					log.info("jobstaticcreate table jobstatic (jobstaticid string,jobname string, jobstatic TEXT,savetime number(10), PRIMARY KEY (jobstaticid))");
				} catch (SQLException e1) {
					log.info("jobstaticcreate table jobstatic (jobstaticid string,jobname string, jobstatic TEXT,savetime number(10), PRIMARY KEY (jobstaticid))",e1);
					e1.printStackTrace();
				}
			}
	}
